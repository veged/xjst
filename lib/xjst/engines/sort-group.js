...
// #### @values {Object} Hash map of all values of all predicates
...
function liftState(source, target, values) {
  ...
  if (target['switch']) {
    target.cases.forEach(function(branch) {
      liftState(source, branch[1], values);
    });

    liftState(source, target['default'], values);
  }
}
...
module.exports = function engine(templates, options, values) {
  ...
  // low priority nodes
  function propagateErrors(tree, values, handlers) {
    ...
    if (tree['switch']) {
      ...
      if (tree['default']['switch'] || tree['default'].stmt !== errStmt) {
        ...
        if (handlers.length > 0) {
          ...
          liftState(tree, handler, values);
        }
      }

      tree.cases.forEach(function(branch) {
        propagateErrors(branch[1], values, [].concat(handlers));
      });
      ...
      propagateErrors(tree['default'], values, [].concat(handlers));
    }
  ...
  return tag(propagateErrors(group(templates), values));
};
