...
// #### @initialState {State} (internal) Initial state, empty by default
// #### @values {Object} Hash map of values of each each predicate
// Returns optimized tree (via sort&group algorithm)
//
module.exports = function engine(templates, options, initialState, values) {
  ...
  function group(templates, state) {
    ...
    tags.reduce(function(result, tag) {
      ...
      var inevitable;
      result.cases.some(function(branch) {
        if (state.has(tagStr, utils.stringify(branch[0]))) {
          inevitable = branch[1];
          return true;
        }
      });
      ...
      if (inevitable && inevitable['switch']) return inevitable['default'];
