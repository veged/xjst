...
//
utils.dispatch = function dispatch(callback) {
  // Apply "synchronously" if no callback was passed
  if (typeof callback !== 'function') {
    var reqq = dispatch.reqq,
        resq = dispatch.resq,
        result;
    ...
    applySync.call(this, function(r) {
      result = r;
    });
    dispatch.reqq = reqq;
    dispatch.resq = resq;
  ...
  while (reqq.length !== 0 || resq.length !== 0) {
    if (reqq.length !== 0) {
      ...
      (function(item) {
        applySync.call(item.self, function(r) {
          item.res = r;
          resq.push(item);
        });
      })(item);
    ...
    if (resq.length !== 0) {
      var item = resq.shift();
      item.callback.call(item.self, item.res);
    }
  }
