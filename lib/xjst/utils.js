...
// return value
//
utils.reduceTree = function reduceTree(tree, fn, acc, visited, stack) {
  if (!stack) stack = [];
  if (!visited) visited = {};
  ...
  if (tree.tag) {
    stack.push(tree);
    acc = fn(acc, tree, stack);

    acc = tree.cases.reduce(function(acc, c) {
      return utils.reduceTree(c.body, fn, acc, visited, stack);
    }, acc);

    if (tree.fallback) {
      acc = utils.reduceTree(tree.fallback, fn, acc, visited, stack);
    }
    stack.pop();
  } else {
    acc = fn(acc, tree, stack);
