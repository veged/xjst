...
utils.run = function run(templates, context) {
  ...
  function oninit(cb) {
    if (context.$init) {
      ...
      if (context.$context && !context.$context.skipApplyNext) {
        context.$context.skipApplyNext = function(context, cb) {
          var old = context.$ignore.slice();
          context.$ignore.length = 0;
          var res = cb.call(context);
          for (var i = 0; i < old.length; i++)
            context.$ignore.push(old[i]);
          return res;
        };
      }
