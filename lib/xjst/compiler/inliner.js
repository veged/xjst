...
Inliner.inline = function inline(ast) {
  return new Inliner().run(ast);
};

Inliner.prototype.run = function run(ast) {
...
Inliner.prototype.inlineClosure = function inlineClosure(node) {
  ...
  if (stmts.length === 1 && stmts[0].type === 'ReturnStatement') {
    ...
    result = { type: 'ExpressionStatement', expression: stmts[0].argument };
  } else {
...
Inliner.prototype.isNonReturn = function isNoReturn(ast) {
  if (!ast)
    return true;

  if (ast.type === 'BlockStatement')
    return ast.body.every(isNonReturn);
