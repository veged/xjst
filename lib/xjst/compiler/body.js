...
Body.prototype.inlineClosure = function inlineClosure(ast) {
...
Body.prototype.rollOutApply = function rollOutApply(ast, type, changes) {
  ...
  var res = new Body(this.compiler, this.compiler._translate2({
    templates: templates,
    other: [],
    init: []
  }, true));
...
Body.prototype.rollOutLocal = function rollOutLocal(ast, changes, body) {
  ...
  var noreturn = false;
  if (body.type === 'FunctionExpression') {
    function isNonReturn(stmt) {
      if (!stmt)
        return true;

      if (stmt.type === 'BlockStatement')
        return stmt.body.every(isNonReturn);

      if (stmt.type === 'IfStatement')
        return isNonReturn(stmt.consequent) && isNonReturn(stmt.alternate);

      if (stmt.type === 'WhileStatement' ||
          stmt.type === 'DoWhileStatement' ||
          stmt.type === 'ForStatement' ||
          stmt.type === 'ForInStatement')
        return isNonReturn(stmt.body);
    ...
    } else if (stmts.every(isNonReturn)) {
      // local()(function() { non-return* })
      body = stmts;
      noreturn = true;
    } else {
      body = {
