...
Compiler.prototype.generate = function generate(code) {
  if (this.options['no-opt'] || this.options.optimize === false) {
    var initCount = (code.match(/xjst\$oninit/g) || []).length;
    return 'var xjst$initLeft = ' + initCount + ';\n' +
           utils.run.toString() + ';\n' +
           ...
           '} catch (e) {\n' +
           '  if (e !== "xjst$init:done") throw e;\n' +
           '}\n' +
           'function templates(template, local, apply, ' +
                               'applyNext, xjst$oninit) {\n' +
           code +
           '};';
  }
