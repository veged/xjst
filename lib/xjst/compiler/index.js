...
Compiler.prototype.generate = function generate(code, options) {
  ...
  return escodegen.generate(this.translate(ast, options));
};
...
Compiler.prototype.sortGroup = function sortGroup(templates) {
  ...
  out = groups.reduce(function(acc, group) {
    ...
    var subgroups = [];
    subgroups.push(pairs.reduce(function(acc, pair) {
      if (acc.length === 0) return [ pair ];

      if (acc[0].predicate.valueId !== pair.predicate.valueId) {
        subgroups.push(acc);
        return [ pair ];
      }
    ...
    pairs = subgroups.reduce(function(acc, subgroup) {
...
Compiler.prototype.render = function render(program) {
