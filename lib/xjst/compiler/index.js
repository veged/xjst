...
function Compiler(options) {
  ...
  this.applyFlags = 0;
  this.jailIndex = 0;
  this.inlineableSize = 60000;
  this.inlineableApplySize = 150000;
  this.inlineDepth = 0;
  this.maxInlineDepth = 1;
...
Compiler.prototype.probeRenderCache = function probeRenderCache(templates) {
  var key = this._renderCacheKey(templates);
  return this.renderCacheMap[key];
};

Compiler.prototype.renderCache = function renderCache(templates, body) {
  var key = this._renderCacheKey(templates);
  this.renderCacheMap[key] = body;
};

Compiler.prototype.splitBodies = function splitBodies(program) {
  var self = this;
  function split(bodies, size) {
    // Depth-First
    bodies.forEach(function(body) {
      // Skip maps
      if (body instanceof Map) {
        split(body.getChildren(), 0);
        return;
      }

      var children = body.getChildren();
      var local = split(children, size) + body.selfSize();

      if (body.shareable && size + local >= self.inlineableSize) {
        self.shareBody(body);
      } else {
        size += local;
      }
