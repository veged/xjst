      ...
      // create ghosts and render everything again
        ...
        if (unique) {
          result.node.cases.forEach(function(branch) {
            tag[utils.stringify(branch[0])] = branch[0];
          });
    ...
    if (forceRecompile) {
      ...
      var newTemplates = [],
          offset = ghosts.reduce(function(offset, ghost) {
            var head = templates.slice(offset, ghost.id + 1),
                template = templates[ghost.id];

            // Push every template before template and template itself
            ...
            Object.keys(ghost.tags).forEach(function(tag) {
              ...
              Object.keys(tag).forEach(function(value) {
                value = tag[value];

                newTemplates.push([
                  template[0].concat([[tagId, tag, value]]),
                  template[1].slice()
                ]);
              });
            });
      ...
      tree = engine(newTemplates, options, engineOptions);
