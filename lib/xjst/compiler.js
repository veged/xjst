var xjst = require('../xjst'),
    ...
    XJSTLocalOptimizer = xjst.ometa.XJSTLocalOptimizer;
...
exports.generate = function generate(templatesAndOther, options) {
  ...
  function omitLocals(tree) {
    reduceTree(tree, function(acc, node) {
      if (node.switch) return;

      node.stmt = XJSTLocalOptimizer.match(node.stmt, 'trans');
    });
    return tree;
  }
  ...
  var engine = xjst.engines[options.engine] || xjst.engines.fullgen,
      body = [
        ...
        serializeTop(omitLocals(optimizeRecursion(
