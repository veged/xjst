...
exports.generate = function generate(ast, options) {
  ...
  if (!options.bump && process.env.XJST_BUMP_PREDICATE) {
    options.bump = new RegExp(process.env.XJST_BUMP_PREDICATE, 'g');
  }
  ...
  if (ast._extensions && ast._extensions.length) {
    ...
    ast._extensions.forEach(function(ext) {
      if (/^[a-z$_][a-z0-9$_]*$/gi.test(ext))
        body.push('  ctx.' + ext + ' = null;');
      else
        body.push('  ctx[' + JSON.stringify(ext) + '] = null;');
    });
    body.push('}');
  }
