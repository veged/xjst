...
exports.generate = function generate(templatesAndOther, options) {
  ...
  function traverse(i, j, predicMemo) {
    function addNode(node, memo) {
      if (node.predic) {
        node.predic.memo = utils.intersect(node.predic.memo, memo);
      } else {
        node.predic = { memo: memo };
      }
  ...
  function optimizeRecursion(tree) {
    ...
    reduceTree(tree, function(acc, o) {
      var r = XJSTLocalAndApplyCompiler.match(o.stmt, 'topLevel');
      console.log(r);
      return acc;
    }, []);
