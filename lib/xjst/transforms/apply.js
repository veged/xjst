...
exports.process = function process(tree, source, options, recompiled) {
  var templates = source,
      ...
      trueId = options.identifier.identify(['get', 'true']);
    ...
    seq.forEach(function(op) {
      ...
      // Remove predicates if their value is already known
        ...
        var predicates = template[0].filter(function(predicate) {
          if (predicate[0] !== trueId  && state[predicate[0]]) {
            changed = true;
            return false;
          }
          return true;
        ...
        // All predicates was filtered out - replace add `true === true`
      ...
      // separate tree
      if (specialized.length === templates.length) return;

      // Generate!
      op[1].code = options.serializer.fnList.add(
        options.identifier.generate(),
